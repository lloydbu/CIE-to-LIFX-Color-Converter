...
//
//  Also converts CIE 1931 x,y to RGB values suited to Osram Lightify bulbs.
//
//  The conversion lookup data in LIFXData50 x 50.plist and OsramData 40 x 40.plist is acceptable but not ideally accurate.
...
- (id)init {
	if ((self = [super init])) {
		...
		path= [[NSBundle mainBundle] pathForResource:@"LIFXData 50 x 50" ofType:@"plist"];
		...
		if (fileExists)
		{
			...
			{
				self.gridLIFX = [ NSKeyedUnarchiver unarchiveObjectWithFile:[url path]];
			}  //url OK
		...
		path= [[NSBundle mainBundle] pathForResource:@"OsramData 40 x 40" ofType:@"plist"];
		...
		if (fileExists)
		{
			...
			if (url != nil && [url isFileURL])
			{
				self.gridOsram = [ NSKeyedUnarchiver unarchiveObjectWithFile:[url path]];
			}  //url OK
...
+ (NSArray *)colorPointsForOsramModel:(NSString*)model
{
	NSMutableArray *colorPoints = [NSMutableArray array];

	[colorPoints addObject:[NSValue valueWithCGPoint:CGPointMake(kOsramRedX, kOsramRedY)]];     // Red
	[colorPoints addObject:[NSValue valueWithCGPoint:CGPointMake(kOsramGreenX, kOsramGreenY)]];     // Green
	[colorPoints addObject:[NSValue valueWithCGPoint:CGPointMake(kOsramBlueX, kOsramBlueY)]];     // Blue
...
+(CGPoint) xy:(CGPoint)P xyPinnedForOsramModel:(NSString*)model
{

	return [LIFXConvert xy:P pinnedForPolygon: [LIFXConvert colorPointsForOsramModel:model]];
...
-(CGPoint) OsramRGfromX:(float)x Y:(float)y
{
	...
	CGPoint clamped = [LIFXConvert xy:CGPointMake(x, y) xyPinnedForOsramModel:nil];

	XY_and_3points_to_barycentrics( Complex(clamped.x, clamped.y),

								   Complex(kOsramRedX,   kOsramRedY),
								   Complex(kOsramGreenX, kOsramGreenY),
								   Complex(kOsramBlueX,  kOsramBlueY),
								   &b1, &b2, &b3);


	CGPoint hs = [self.gridOsram get_ramped_point_from_unitary:CGPointMake(b1, b2) fixWrapX:NO];
...
-(void) OsramR:(float*)R G:(float*)G B:(float*)B fromX:(float)x Y:(float)y
{
	...
	if (self.gridOsram) {
		...
		//		r = [MathHappy getRandom1];		a test
		//		g = [MathHappy getRandom1];

		CGPoint rg = [self OsramRGfromX:x Y:y];

		r = rg.x;
		g = rg.y;


		//r+g+b = 1, always. Osram separates chromaticity from brightness this way.
		CLAMP(r,0,1)
		CLAMP(g,0,1)

		b = 1.0-r-g;

		//normalize so the largest component is 1.0

		float largest = MAX(MAX(r,g),b);
...
-(CGPoint) HSfromX:(float)x Y:(float)y
{
	...
	CGPoint hs = [self.gridLIFX get_ramped_point_from_unitary:CGPointMake(b1, b2) fixWrapX:YES];
